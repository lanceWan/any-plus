...
Eof;
	...
	protected function getActionButtons($id)
	{
		...
		if (haspermission('menucontroller.show')) {
			$action .= '<a href="javascript:;" class="btn btn-xs tooltips showInfo" data-href="'.route('menu.show',  $encodeId).'" data-toggle="tooltip" data-original-title="查看"  data-placement="top"><i class="fa fa-eye"></i></a>';
		}
		if (haspermission('menucontroller.edit')) {
			$action .= '<a href="javascript:;" data-href="'.route('menu.edit', $encodeId).'" class="btn btn-xs tooltips editMenu" data-toggle="tooltip"data-original-title="编辑"  data-placement="top"><i class="fa fa-edit"></i></a>';
		}
	...
	{
		$canCreateMenu = haspermission('menucontroller.create');

		$title = $canCreateMenu ?  'Welcome ！':'Sorry ！';
		$desc = $canCreateMenu ? '你可以操作左侧菜单列表，或者点击下面添加按钮新增菜单！':'暂无权限添加菜单';
		$createButton = $canCreateMenu ? '<br><a href="javascript:;" class="btn btn-primary m-t create_menu">创建菜单</a>':'';
		return <<<Eof
